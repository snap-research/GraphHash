...
def train_model(
    ...
    for inputs in tqdm(train_loader):
        ...
        # Get the shape of neg_item_id
        shape = item_id.shape

        # # Resample the negative each epoch
        neg_item_id = torch.randint(0, len(item_cluster_tensors), shape)

    
        user_cluster_ids = user_cluster_tensors[user_id]
        item_cluster_ids = item_cluster_tensors[item_id]
        neg_item_cluster_ids = item_cluster_tensors[neg_item_id]
        ...
        user_embedding, negative_item_embedding = model(
            user_cluster_ids,
            neg_item_cluster_ids,
            )
        
    


        loss = criterion(user_embedding, positive_item_embedding, negative_item_embedding)
        loss.backward()
        optimizer.step()
...
def evaluate_model(
    hash_type, model, val_loader, user_cluster_tensors, item_cluster_tensors, criterion, device
):
    model.eval()
    running_loss = 0.0
    total_num_users = 0
...
def train_and_evaluate_ials(
    ...
    if type(model).__name__ == "LightGCN":
        if len(user_clusters.shape) == 1:
            user_cluster_tensors = torch.arange(len(user_clusters), dtype=torch.int64, device=device)
            item_cluster_tensors = torch.arange(len(item_clusters), dtype=torch.int64, device=device)
        else: 
            user_cluster_tensors = torch.arange(user_clusters.shape[0], dtype=torch.int64, device=device)
            item_cluster_tensors = torch.arange(item_clusters.shape[0], dtype=torch.int64, device=device)
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        if epoch % 10 == 0:
            ...
            print(f'Train Loss:{train_loss}, Val Recall: {val_recall}, Val NDCG: {val_ndcg}')
            wandb.log({
