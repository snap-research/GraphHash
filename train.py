...
from data import UserItemRatingDataset, UserItemRatingDatasetv2, MovieLens20MDataset, FrappeDataset
import os
import hydra
...
def main(config):
    ...
    if dataset not in ["MovieLens20M"]:
        ...
        train_set = MovieLens20MDataset(h5_file_path=f"datasets/{dataset}-processed/train.h5", dataset_key='train')
        val_set = MovieLens20MDataset(h5_file_path=f"datasets/{dataset}-processed/val.h5", dataset_key='val')
        test_set = MovieLens20MDataset(h5_file_path=f"datasets/{dataset}-processed/test.h5", dataset_key='test')
    ...
    if loss == "BCE":
        ...
        if dataset not in ["Frappe"]:
            ...
            user_feature_file_name = (f"datasets/{dataset}-processed/all_user_features.pt")
            all_user_features = torch.load(user_feature_file_name)

            item_feature_file_name = (f"datasets/{dataset}-processed/all_item_features.pt")
    ...
    if hash_type in ["graph", "random", "frequency", "double", "double_frequency", "lsh", "lsh-structure", "spectral", "double_graph"]:
        # Generate GH
        if dataset != "MovieLens20M":
            ...
            biadjacency = sparse.load_npz(f'datasets/{dataset}-processed/biadjacency.npz')
        ...
        if hash_type == "graph":
            ...
            user_clusters, item_clusters = double_graph_hash(biadjacency, resolution,
                                                             num_users_clusters, 
                                                             num_items_clusters, 
                                                             num_total_users, 
                                                             num_total_items)
        ...
        user_clusters = np.load(f'datasets/{dataset}-processed/user_spectral_clusters.npy')
        item_clusters = np.load(f'datasets/{dataset}-processed/item_spectral_clusters.npy')
    ...
    if loss in ['BPR', 'DAU', "iALS"]: 
        if model_name == "LightGCN":
            ...
            model = instantiate(config.model, user_vocab_size=num_users_clusters, item_vocab_size=num_items_clusters, 
                                biadjacency=biadjacency,
            ...
            model = instantiate(config.model, user_vocab_size=num_users_clusters, item_vocab_size=num_items_clusters, embedding_dim=embedding_dim).to(device)
        ...
        if dataset not in ["MovieLens20M"]:
            if dataset == 'Frappe':
                ...
                model = instantiate(config.model,   
                                    ...
                                    input_item_feature_size=item_feature_dim).to(device)
