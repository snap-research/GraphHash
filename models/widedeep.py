...
class WideDeep(nn.Module):
    def __init__(self, 
        ...
        self.wide = Wide(feature_dim=2 * embedding_dim + dense_feature_size, embedding_dim=embedding_dim)
...
class WideDeepxL(nn.Module):
    def __init__(self, 
        ...
        self.wide = Wide(feature_dim= 3  * embedding_dim + dense_feature_size, embedding_dim=embedding_dim)
        self.deep = MLP(input_size= 3 * embedding_dim + dense_feature_size,
                        hidden_size=embedding_dim*2,
                        output_size=embedding_dim,
                        hidden_layers=3)
        self.together = nn.Linear(2 * embedding_dim, 1)
    ...
    def forward(self, input): 
        ...
        combined_in = torch.cat([user_id_embeddings, item_id_embeddings, tag_embeddings, dense_features], dim=-1)
        wide_out = self.wide(combined_in)
        deep_out = self.deep(combined_in)
...
class WideDeeps(nn.Module):
     def __init__(
        ...
        self.wide = Wide(feature_dim= (self.sparse_feature_size + 2)  * embedding_dim, embedding_dim=embedding_dim)
     ...
     def forward(self, input):
        ...
        combined_in = torch.concat([user_id_embeddings,
                              item_id_embeddings,
