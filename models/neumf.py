...
class NeuMF(BaseRecommender):
    ...
    def get_scores(self, hash_type, device, user_id_hashed, item_id_hashed):
        ...
        mf_output = user_id_embeddings.unsqueeze(1) * item_id_embeddings.unsqueeze(
            0
        )  # shape: (# users, # items, embed_dim)
        mf_output = mf_output.view(-1, self.embedding_dim)

        # Expand dimensions for broadcasting
        user_expanded = user_mlp_e.unsqueeze(1).expand(
            user_mlp_e.shape[0], item_mlp_e.shape[0], self.embedding_dim
        )  # Shape (N, M, d)
        item_expanded = item_mlp_e.unsqueeze(0).expand(
            user_mlp_e.shape[0], item_mlp_e.shape[0], self.embedding_dim
        )  # Shape (N, M, d)

        # Concatenate user and item embeddings
        pairwise_features = torch.cat(
            (user_expanded, item_expanded), dim=2
        )  # Shape (N, M, 2d)

        # Reshape to (N * M, 2d) to process through the MLP in a single batch
        pairwise_features_reshaped = pairwise_features.view(
        ...
        scores = self.predict_layer(
            torch.cat((mf_output, mlp_output), -1)
        )  # Shape (N * M, 1)
